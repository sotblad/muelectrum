...
from electrum_monetaryunit.bitcoin import (TYPE_ADDRESS, int_to_hex, var_int,
                                   bc_address_to_hash_160, ADDRTYPE_P2PKH,
                                   hash_160_to_bc_address, ADDRTYPE_P2SH)
from electrum_monetaryunit.i18n import _
from electrum_monetaryunit.plugins import BasePlugin, hook
from electrum_monetaryunit.keystore import Hardware_KeyStore, parse_xpubkey
from electrum_monetaryunit.transaction import push_script, Transaction
from ..hw_wallet import HW_PluginBase
from electrum_monetaryunit.util import format_satoshis_plain, print_error, is_verbose
        ...
        try:
            ...
            if self.has_detached_pin_support(self.dongleObject) and not self.is_pin_validated(self.dongleObject) and (self.handler <> None):
                ...
                if self.canAlternateCoinVersions:
                    self.dongleObject.setAlternateCoinVersions(ADDRTYPE_P2PKH,
                                                               ADDRTYPE_P2SH)
        except BTChipException, e:
            if (e.sw == 0x6faa):
    ...
    def checkDevice(self):
        if not self.preflightDone:
            try:
                ...
                if (e.sw == 0x6d00):
                    raise BaseException("Device not in MonetaryUnit mode")
                raise e
...
class LedgerPlugin(HW_PluginBase):
    ...
    def setup_device(self, device_info, wizard):        
        ...
        #client.get_xpub('m')
        client.get_xpub("m/44'/5'") # TODO replace by direct derivation once Nano S > 1.1
