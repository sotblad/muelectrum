...
;Variables
  ...
  !define PRODUCT_NAME "Electrum-MUE"
...
;General
  ...
  ;Name and file
  Name "${PRODUCT_NAME}"
  OutFile "dist/electrum-monetaryunit-${PRODUCT_VERSION}-setup.exe"
...
;Interface Settings
  ...
  !define MUI_ICON "icons\electrum-monetaryunit.ico"
...
Section
  ...
  File /r "dist\electrum-monetaryunit\*.*"
  File "icons\electrum-monetaryunit.ico"
  ...
  CreateShortCut "$DESKTOP\${PRODUCT_NAME}.lnk" "$INSTDIR\electrum-monetaryunit-${PRODUCT_VERSION}.exe" ""

  ;Create start-menu items
  DetailPrint "Creating start-menu items..."
  CreateDirectory "$SMPROGRAMS\${PRODUCT_NAME}"
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\Uninstall.lnk" "$INSTDIR\Uninstall.exe" "" "$INSTDIR\Uninstall.exe" 0
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\${PRODUCT_NAME}.lnk" "$INSTDIR\electrum-monetaryunit-${PRODUCT_VERSION}.exe" "" "$INSTDIR\electrum-monetaryunit-${PRODUCT_VERSION}.exe" 0

  ;Links monetaryunit: URI's to Electrum
  WriteRegStr HKCU "Software\Classes\monetaryunit" "" "URL:monetaryunit Protocol"
  WriteRegStr HKCU "Software\Classes\monetaryunit" "URL Protocol" ""
  WriteRegStr HKCU "Software\Classes\monetaryunit" "DefaultIcon" "$\"$INSTDIR\electrum-monetaryunit.ico, 0$\""
  WriteRegStr HKCU "Software\Classes\monetaryunit\shell\open\command" "" "$\"$INSTDIR\electrum-monetaryunit-${PRODUCT_VERSION}.exe$\" $\"%1$\""
  ...
  WriteRegStr HKCU "${PRODUCT_UNINST_KEY}" "DisplayIcon" "$INSTDIR\electrum-monetaryunit.ico"
