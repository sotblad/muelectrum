...
from electrum_monetaryunit.util import (profiler, parse_URI, format_time,
                                InvalidPassword, NotEnoughFunds)
from electrum_monetaryunit import bitcoin
from electrum_monetaryunit.util import timestamp_to_datetime
...
class SendScreen(CScreen):
    ...
    def set_URI(self, text):
        ...
        try:
            uri = electrum_monetaryunit.util.parse_URI(text, self.app.on_pr)
        except:
            self.app.show_info(_("Not a MonetaryUnit URI"))
            return
    ...
    def do_save(self):
        ...
        from electrum_monetaryunit.paymentrequest import make_unsigned_request, PaymentRequest
    ...
    def do_send(self):
        if self.screen.is_pr:
            ...
            if not address:
                self.app.show_error(_('Recipient not specified.') + ' ' + _('Please scan a MonetaryUnit address or a payment request'))
                return
            if not bitcoin.is_address(address):
                self.app.show_error(_('Invalid MonetaryUnit Address') + ':\n' + address)
                return
            try:
...
class ReceiveScreen(CScreen):
    ...
    def get_URI(self):
        from electrum_monetaryunit.util import create_URI
    ...
    def do_share(self):
        uri = self.get_URI()
        self.app.do_share(uri, _("Share MonetaryUnit Request"))
